...
def select_num(dictionary, num):
    ...
    for i in keys:
        ...
        correct = random.sample(
            dictionary[i][0], random.randint(1, min(num, len(dictionary[i][0])))
        )
...
for question in questions.keys():
    ...
    layout.append(
        [
            sg.Checkbox(x, key=question + " " + x, font=("Helvetica", 15))
            for x in questions[question]["all"]
        ]
    )
    correct_answers = len(questions[question]["correct"]) + correct_answers
layout.append([sg.Button("Submit")])
window = sg.Window(
    f"Quiz:0/{correct_answers}", layout, auto_size_text=True, auto_size_buttons=True
)
while True:
